...
const AeginaElevation = () => {
  ...
  useEffect(() => {
    ...
    // Get polygon rings - need to handle deeper nesting
    const getPolygonRing = (coords) => {
      // Keep unwrapping until we get to an array of [lon, lat] pairs
      let current = coords;
    ...
    console.log('Aegina ring has', aeginaRing.length, 'points');
    console.log('First Aegina point:', aeginaRing[0]);
    console.log('Moni ring has', moniRing.length, 'points');
    ...
    const testLon = 23.49;
    const testLat = 37.72;
    console.log(`Test point (${testLon}, ${testLat}) in Aegina:`, isPointInPolygon(testLon, testLat, aeginaRing));
    ...
    console.log('Points inside islands:', insideCount);
    console.log('Island elevation range:', minElev, 'to', maxElev, 'meters');
    ...
    // Create rectangles for island pixels only
    const rects = [];
    ...
    console.log(`Created ${rects.length} elevation rectangles (islands only)`);
    setElevationRects(rects);
  ...
  return (
    <div style={{ 
      ...
      <div style={{ marginBottom: '20px', fontSize: '18px', fontWeight: 'bold' }}>
        Aegina & Moni Islands - Elevation Map
      </div>
      ...
      <div style={{ marginTop: '20px', fontSize: '12px', color: '#666' }}>
        Light green = low elevation, Dark green = high elevation
      </div>
    </div>
  );
};
