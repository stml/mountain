...
const AeginaElevation = () => {
  ...
  // Resize star canvas to match window
  useEffect(() => {
    const handleResize = () => {
      if (starCanvasRef.current) {
        const size = Math.min(window.innerWidth, window.innerHeight);
        starCanvasRef.current.width = size;
        starCanvasRef.current.height = size;
      }
    };
    handleResize();
  ...
  // Trigger initial star/sky render
  useEffect(() => {
    setSceneReady(true);
  }, []);
  ...
  useEffect(() => {
    ...
    let newStarOpacity = 0;
    if (elevDegrees < -6) {
      newStarOpacity = 1; // Full night - stars fully visible
    } else if (elevDegrees < 0) {
      newStarOpacity = 1 - (elevDegrees + 6) / 6; // Fade out during twilight
    }
    setStarOpacity(newStarOpacity);
    
    // Draw stars on 2D canvas
    if (starCanvasRef.current && newStarOpacity > 0) {
      ...
      const radius = Math.min(width, height) / 2 - 20; // Radius of the star map circle
      ...
      BRIGHT_STARS.forEach(star => {
        ...
        // Stereographic projection - altitude maps to distance from center
        // 90° (zenith) = center, 0° (horizon) = edge
        const r = radius * Math.cos(altitude) / (1 + Math.sin(altitude));
        
        // Azimuth gives angle (0 = North at top, clockwise)
        const x = centerX + r * Math.sin(azimuth);
        const y = centerY - r * Math.cos(azimuth);
        
        // Star size based on magnitude
        const size = Math.max(1.5, 4 - star.mag * 0.8);
        
        // Draw star with glow
        const gradient = ctx.createRadialGradient(x, y, 0, x, y, size * 2);
        gradient.addColorStop(0, `rgba(255, 255, 255, ${newStarOpacity})`);
        gradient.addColorStop(0.3, `rgba(255, 255, 255, ${newStarOpacity * 0.6})`);
        ...
        // Star label for brightest stars
        if (star.mag < 0.5) {
          ctx.font = '10px sans-serif';
          ctx.fillStyle = `rgba(255, 255, 255, ${newStarOpacity * 0.7})`;
          ctx.fillText(star.name, x + size * 2 + 2, y + 3);
        }
      });
      ...
      ctx.strokeStyle = `rgba(255, 255, 255, ${newStarOpacity * 0.3})`;
      ctx.lineWidth = 1;
      ctx.stroke();
      
      // Draw cardinal directions
      ctx.font = '12px sans-serif';
      ctx.fillStyle = `rgba(255, 255, 255, ${newStarOpacity * 0.5})`;
  ...
  useEffect(() => {
    ...
    // Scene background is null (transparent) - sky color is on the page background
  ...
  return (
    <div style={{ position: 'relative', width: '100vw', height: '100vh', overflow: 'hidden', backgroundColor: skyColor }}>
      {/* Star map canvas - behind everything */}
      <canvas
        ref={starCanvasRef}
        width={800}
        height={800}
