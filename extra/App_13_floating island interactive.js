...
const RETURN_SPEED = 0.02; // How quickly it returns to base spin speed
...
const AeginaElevation = () => {
  ...
  useEffect(() => {
    ...
    // Sea plane - flat plane at zero elevation
    ...
    const handlePointerMove = (e) => {
      ...
      if (newTiltX > -Math.PI / 2 && newTiltX < -Math.PI / 6) {
    ...
    const handleWheel = (e) => {
      ...
      // Clamp zoom range
      camera.position.z = Math.max(4, Math.min(20, newZ));
    ...
    const animate = () => {
      ...
      if (!isDragging) {
        ...
        if (Math.abs(tiltVelocity) > 0.0001) {
          ...
          // Clamp tilt
          terrain.rotation.x = Math.max(-Math.PI / 2, Math.min(-Math.PI / 6, terrain.rotation.x));
