...
useEffect(() => {
  ...
  if (existingCanvas) {
    ...
    // Create plane with segments matching elevation data
    const planeWidth = 8;
    const planeHeight = 5.5;
    
    // Create plane with one vertex per elevation point
    geometry = new THREE.PlaneGeometry(
      planeWidth, 
      planeHeight, 
      cols - 1,  // segments in X
      rows - 1   // segments in Y
    );
    ...
    for (let i = 0; i < positions.count; i++) {
      ...
      // Map 3D position back to lat/lon
      // x ranges from -planeWidth/2 to +planeWidth/2
      // y ranges from -planeHeight/2 to +planeHeight/2
      const lonNorm = (x + planeWidth / 2) / planeWidth;   // 0 to 1
      const latNorm = (y + planeHeight / 2) / planeHeight; // 0 to 1
      
      const lon = minLon + lonNorm * (maxLon - minLon);
      const lat = minLat + latNorm * (maxLat - minLat);
      
      // Find corresponding elevation grid point
      const col = Math.floor(lonNorm * (cols - 1));
      const row = Math.floor((1 - latNorm) * (rows - 1)); // Flip Y
      
      // Check if point is inside islands
      const inAegina = isPointInPolygon(lon, lat, aeginaRing);
      const inMoni = isPointInPolygon(lon, lat, moniRing);
      ...
      if ((inAegina || inMoni) && row >= 0 && row < rows && col >= 0 && col < cols) {
        ...
        if (elevation < 0) elevation = 0; // Clip negative values
      }
      
      // Scale elevation (divide by 100 to get reasonable height)
      const z = elevation / 500;
