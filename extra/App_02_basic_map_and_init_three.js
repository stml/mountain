...
useEffect(() => {
  ...
  let renderer, geometry, material, cube;
  ...
  if (existingCanvas) {
    ...
    // Recreate the cube (since we don't have reference to old one)
    geometry = new THREE.BoxGeometry(2, 2, 2);
    material = new THREE.MeshPhongMaterial({ color: 0x7a9f35 });
  ...
  return () => {
    ...
    geometry.dispose();
    material.dispose();
    // Don't dispose renderer or remove canvas - let it persist
  };
}, []);
